...
class functions():
    ...
    def findbyAttribute(self, query):
        ...
        if "id" in res:
            ret[0] = (self.data["ID"]== res["id"])
        if "name" in res:
            ret[1] =  (self.data["Name"]== str(res["name"]))
        if "event" in res:
            ret[2] =  (self.data["Event name"]== str(res["event"]))
        if "component" in res:
            ret[3] =  (self.data["Component"]== str(res["component"]))
        return self.data.loc[ret[0] & ret[1] & ret[2] & ret[3]]
    
    def groupbyComponent(self, query):
        query = query.split("=")
        if "component" in query:
            lstEnrolled = pandas.DataFrame(self.data.loc[self.data["Component"] == query[1]]["ID"].unique())
        if "event" in query:
            lstEnrolled = pandas.DataFrame(self.data.loc[self.data["Event name"] == query[1]]["ID"].unique(), columns=[["ID"]])
        if "object" in query:
            lstEnrolled = pandas.DataFrame(self.data.loc[self.data["Object"] == query[1]]["ID"].unique(), columns=["ID"])
        return lstEnrolled
